package com.custodia.supply.models.dao.request;
...
public interface IRequestDao extends PagingAndSortingRepository<Request, Long>, CrudRepository<Request, Long> {
	...
	/*
	 * === Brings only the request items by User id === Allows to have empty fields
	 * by validating from RequestRow DTO Helps to have no assigned attributes from
	 * CustomerAccount when User first register.
	 */
	@Query(value = """
			  select new com.custodia.supply.models.dto.request.RequestViewDTO(
			    r.id,
			    r.description,
			    r.additionalItems,
			    r.status,
	...
	@Query("select distinct r from Request r" + " join fetch r.user " + "left join fetch r.items ri "
			+ "left join fetch ri.supplyItem si " + "join fetch r.shipTo cs " + "join fetch cs.customer ca "
			+ "where r.id = :id")
	...
	@Query("""
			  select distinct r from Request r
			    ...
			    left join fetch i.supplyItem si
			  where r.user.id in :userIds
			""")
	List<Request> fetchItemsForRequests(@Param("userIds") List<Long> userIds);
