package com.custodia.supply.controllers.user;
...
public class UserController {
	...
	@Autowired
	private IPageableService<User> pageableUser;
	...
	@Autowired
	private UserMapperFormImpl userForm;
	
	@Autowired
	private UserMapperViewImpl userView;
	...
	@GetMapping("/tree")
	public ResponseEntity<List<UserViewDTO>> getUsersTree() {
		List<UserViewDTO> body = userService.exportUsersTree();
	...
	public String list(@RequestParam(name = "page", defaultValue = "0") int page, Model model,
		...
		Page<User> users = pageableUser.findAll(pageRequest);
		// Map to handle nulls in the list - works good!
		//Page<UserDetailView> usersDTO = users.map(UserDetailView::of);
		// new implementation using component mapper
		
//		Page<UserViewDTO> usersDTO = users.map(userView::toDTO);
		Page<UserViewDTO> usersDTO = users.map(UserMapper::toDetail);
	...
	public String view(@RequestParam(name = "page", defaultValue = "0") int page,
	    ...
	    //=== Old version of Mapper ===
//	    UserViewDTO user = UserMapper.toView(u);
	    //=== New Mapper version === 
//	    UserViewDTO user = userView.toDTO(u);
	    //=== Testing new Mapper Version ===
	    UserViewDTO user = UserMapper.toDetail(u);
...
//		UserFormDTO user = UserMapper.toForm(u);
		//=== New Mapper version ===
		UserFormDTO user = userForm.toDTO(u);
